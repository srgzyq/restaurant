...
IGNORE_NAME = [u'基础信息']
SUM_INCOME_TITLE_LIST = [u'日期', u'星期', u'刷卡',
                         u'现金', u'支付宝', u'微信', u'大众闪惠', u'糯米',u'美团']
PAY_BUY_LIST = [u'刷卡', u'现金', u'支付宝', u'微信', u'大众闪惠', u'糯米',u'美团']
                ...
                for row_index in range(1, sheet_rows):
                    ...
                    for col, value in enumerate(line_data_value):
                        ...
                        if from_xls_sheet.row_types(row_index)[col] == 3:
                            ...
                            sheet_xls.write(
                                row_index + xls_sheet_content_index[sheet_name], col, value, style)
                        else:
                            sheet_xls.write(
                                row_index + xls_sheet_content_index[sheet_name], col, value)

                # 偏移量除去第一行
                xls_sheet_content_index[sheet_name] += sheet_rows - 1

        # 日志输出:
        myLogging.logging.info(file_name + " merge to" +
                               to_file_name + " succefull...")
...
def sumWeeklyIncomeXlsFile(from_file_name):
    ...
    for row_index in range(1, sheet_rows):
        ...
        if weekly_dic.get(line_data_value[0]) is None:
        ...
        try:
            weekly_dic[line_data_value[0]][
                line_data_value[8]] += line_data_value[9]
        except (ValueError, KeyError):
            error_key = line_data_value[8]
            myLogging.logging.info(
                "key " + error_key + " PAY_BUY_LIST:" + " ".join(PAY_BUY_LIST))
    ...
    for key, value in weekly_dic.items():
        ...
        for inncome_type, value in value.items():
            try:
                ...
                myLogging.logging.info(
                    "from_file_name:" + from_file_name + " inncome_type " + inncome_type + "error")
...
def saveBaseInfoSumWeeklyXls(from_file_name, to_file_name):
    pass
...
def testMergeMoreXlfFileData():
    ...
    from_file_names = ["./data/2016.4.25.xlsx", "./data/2016.4.26.xlsx", "./data/2016.4.27.xlsx",
                       "./data/2016.4.28.xlsx", "./data/2016.4.29.xlsx", "./data/2016.4.30.xlsx"]
    to_file_name = u"./data/4月/第四周.xls"
...
def testSumWeeklyIncomeXlsFile():
    ...
    from_file_name = u"./data/4月/第四周.xls"
    to_file_name = u"./data/4月/第四周收入汇总.xls"
